        ...
        <aside className="lg:w-80 border-b lg:border-b-0 lg:border-r border-border p-4 flex flex-col gap-4 overflow-y-auto shrink-0">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <div className="w-5 h-5 rounded bg-secondary flex items-center justify-center text-xs font-mono font-bold text-secondary-foreground">
                1
              </div>
            ...
            <UploadBox
              label="Twibbon Layer"
              sublabel="Always behind — fixed stacking"
              media={bottomLayer}
              onMediaChange={setBottomLayer}
              icon="bottom"
            />
          </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <div className="w-5 h-5 rounded bg-primary/20 flex items-center justify-center text-xs font-mono font-bold text-primary">
                2
              </div>
            ...
            <UploadBox
              label="User Layer"
              sublabel="Drag, resize, rotate on canvas"
              media={topLayer}
              onMediaChange={setTopLayer}
              icon="top"
            />
          </div>
          ...
          <div className="mt-auto pt-4 border-t border-border space-y-2 text-xs text-muted-foreground">
            <p>
              ...
              {hasAnimation ? "MP4 (animated content detected)" : "PNG / JPG (static)"}
            </p>
            <p className="text-muted-foreground/60">
              Scroll on canvas to zoom • Drag to reposition
            </p>
          </div>
        </aside>
